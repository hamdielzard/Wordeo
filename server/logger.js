// Useful to log information about the server
// such as when it starts, when it receives a request, and when it crashes
...
const logs = [];
const logSockets = true; // added boolean to decide whether to log socket logs or not
...
const logger = {
  ...
  socket: (message) => {
    ...
    if (logSockets) { // check if logSockets is true before logging socket logs
      ...
      // exportLogs();
      // TODO: Turn this on after fixing client spam,
      // otherwise it will destroy your hard drive with constant writes :)
    }
  },
};
