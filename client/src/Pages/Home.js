    ...
    async function requestNewLobby(gameMode) {
        ...
        if (userNameExists && displayNameExists) {
            ...
            if (response.status !== 200) {
                ...
                if (data.message.includes('created successfully') && data.userName === userName) {
...
function loginButton() {
    ...
    if (userNameExists && displayNameExists) {
        ...
        return (
            <Button transparent={true} label="Not signed in" onClick={() => { window.location = "/account/signin" }} type="secondary" size={32} />
        )
    }
}
